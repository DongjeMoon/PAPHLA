...
class LossComputer:
    '''
    Adapted from Shiori Sagawa*, Pang Wei Koh*, Tatsunori Hashimoto, and Percy Liang
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        if yhat.dim() == 1 or (yhat.dim()==2 and yhat.size(1)==1):
            # Binary branch
            pred = (yhat.view(-1) >= 0).float()     # logit >= 0 ↔ prob >= 0.5
            group_acc, group_count = self.compute_group_avg((pred == y.float()).float(), group_idx)
        else:
            # Multi-class as original
            group_acc, group_count = self.compute_group_avg((torch.argmax(yhat,1)==y).float(), group_idx)        
